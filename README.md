...
1. **地块认养与 GIS 展示**：地图展示地块分布，标注空闲/已认养/待释放/转交审核中状态，展示面积、土壤类型、日照条件，在线认养。
2. **地块转交申请流程**：认养人提交转交申请（填接管理由）→ 地块进入转交审核中（其他居民不可认养）→ 管理员核准后清空原认养关系回到共享池；待处理期间本人可撤回，管理员可写意见驳回；核准与撤回并发时只接受先完成的一次，后到的提示已处理。
...
| 地块转交申请 PlotTransferRequest | `plot_transfer_requests` | `model/plot_transfer_request.go`、`repository/plot_transfer_repository.go`、`service/plot_transfer_service.go`、`handler/plot_transfer_handler.go`、`router/plot_transfer.go` | `api/plotTransfer.ts`、`stores/plotTransfer.ts`、`pages/PlotTransfers.vue` |
...
| PlotStatus 地块状态 | available / adopted / harvested / transfer_pending | `constants/enums.go`、`model/plot.go`、`dto/plot_dto.go`、`service/plot_service.go`（认养/释放状态机）、`service/plot_transfer_service.go`（转交申请状态流转）、`repository/plot_repository.go`（过滤）、`util/formatters.go`、`log_templates.go`、`database/database.go`（种子数据）、`api/openapi.yaml`、前端 `constants/index.ts`（PlotStatusMeta 徽标）、`pages/PlotMap.vue`（按钮显隐/地图着色） |
| TransferStatus 转交申请状态 | pending / approved / rejected / cancelled | `constants/enums.go`、`model/plot_transfer_request.go`、`dto/plot_transfer_dto.go`、`service/plot_transfer_service.go`（pending 才能核准/驳回/撤回的状态机）、`repository/plot_transfer_repository.go`（过滤/待处理唯一性校验）、`handler/plot_transfer_handler.go`、`util/formatters.go`（TransferStatusText）、`log_templates.go`、`error_codes.go`（CodeTransferNotPending 等）、前端 `constants/index.ts`（TransferStatusMeta）、`pages/PlotTransfers.vue`（筛选/按钮显隐） |
...
| POST | `/plots/:id/adopt` | 认养地块（事务 + FOR UPDATE；转交审核中不可认养） | 登录 |
| POST | `/plots/:id/release` | 直接释放地块（防误点，认养人须走转交申请） | 管理员 |
| POST | `/plots/:id/transfer-requests` | 认养人提交转交申请（填接管理由） | 登录（认养人本人） |

### 地块转交申请
| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/transfer-requests` | 申请列表（`?status=`；管理员看全部，认养人看自己的） | 登录 |
| GET | `/transfer-requests/:id` | 申请详情（申请原因/处理意见/当前状态） | 本人/管理员 |
| POST | `/transfer-requests/:id/approve` | 核准（清空原认养关系，地块回到共享池） | 管理员 |
| POST | `/transfer-requests/:id/reject` | 驳回（必填处理意见，地块恢复申请前状态） | 管理员 |
| POST | `/transfer-requests/:id/cancel` | 撤回待处理申请（与核准并发只接受先完成的一次） | 申请人本人 |
...
# 4.1 认养人提交地块转交申请（填接管理由，地块进入转交审核中）
curl -s -X POST http://localhost:29516/api/v1/plots/2/transfer-requests \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"reason":"农忙无暇打理，申请转交给其他居民"}'

# 4.2 转交申请列表（管理员看全部，认养人看自己的）
curl -s "http://localhost:29516/api/v1/transfer-requests?status=pending" \
  -H "Authorization: Bearer $TOKEN"

# 4.3 管理员核准（清空原认养关系，地块回到共享池）
curl -s -X POST http://localhost:29516/api/v1/transfer-requests/1/approve \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"review_comment":"同意，感谢本季照料"}'

# 4.4 管理员驳回（必填处理意见）/ 申请人撤回
curl -s -X POST http://localhost:29516/api/v1/transfer-requests/1/reject \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"review_comment":"作物即将成熟，建议本季结束后再申请"}'
curl -s -X POST http://localhost:29516/api/v1/transfer-requests/1/cancel \
  -H "Authorization: Bearer $TOKEN"
